...
import org.ubstorm.service.parser.formparser.data.BandInfoMapData.IMethodBar;
import org.ubstorm.service.utils.common;
	
public class BandInfoMapDataSimple {
	...
	ArrayList<HashMap<String, Object>> mTableProperties;
	...
	ArrayList<HashMap<String, Object>> children = new ArrayList<HashMap<String,Object>>();		// 생성될 아이템들의 정보를 담아둘 배열(한Row에 생성될 아이템의 정보를 담는 객체)
	...
	public void setChildren(ArrayList<HashMap<String, Object>> children) {
	...
	public BandInfoMapDataSimple( HashMap<String, Object> _item ) {
		super();
		// TODO Auto-generated constructor stub
...
//				setColumnAr(value); } } );
	...
	{
		propertyMapping.get("className").callMethod( URLDecoder.decode( _item.get("className").toString(), "UTF-8" ) );
		propertyMapping.get("id").callMethod( URLDecoder.decode( _item.get("id").toString(), "UTF-8" ) );
		...
		for( String _key : _item.keySet() )
		{
			...
			else if( _key.equals("columns") )
			{
				if( this.mClassName.equals(BandInfoMapData.GROUP_HEADER_BAND ))
				{
					setColumnsArrayList( _item.get(_key) );
				}
				else if(  this.mClassName.equals(BandInfoMapData.GROUP_FOOTER_BAND ) )
				{
					setFooterColumnsArray( _item.get(_key) );
				}
			}
			else if( _key.equals("sort") )
			{
				try {
					JSONArray _jr = (JSONArray) _item.get(_key);
					JSONObject _jObj;
					Object _temp;
					String _tempValue = "";
					int _size = _jr.size();
					mSort = new ArrayList<HashMap<String,String>>();
					HashMap<String,String> _hMap;
					//[{"isNumeric":1,"orderBy":false,"column":"col_1"}]
					
					for( int i = 0; i < _size; i++ )
					{
						_hMap = new HashMap<String, String>();
						_jObj = (JSONObject) _jr.get(i);
						_temp = _jObj.get("isNumeric");
						if( _temp != null )
						{
							_tempValue = ( _temp.toString().equals("1")|| _temp.toString().equals("true") )?"true":"false";
							_hMap.put("isNumeric", _tempValue );
						}
						
						_temp = _jObj.get("orderBy");
						if( _temp != null )
						{
							_tempValue = ( _temp.toString().equals("1")|| _temp.toString().equals("true") )?"true":"false";
							_hMap.put("orderBy", _tempValue );
						}

						_temp = _jObj.get("column");
						if( _temp != null )
						{
							_tempValue = _temp.toString();
							_hMap.put("column", _tempValue );
						}
			...
			else
			{
				...
				{
					try {
						propertyMapping.get(_key).callMethod( URLDecoder.decode( _item.get(_key).toString(), "UTF-8" ) );
    ...
    public BandInfoMapDataSimple cloneBandInfo()
    {
    	BandInfoMapDataSimple retBandData = new BandInfoMapDataSimple();
	...
	public ArrayList<HashMap<String, Object>> getTableProperties() {
		return mTableProperties;
	}

	public void setTableProperties(ArrayList<HashMap<String, Object>> mTableProperties) {
		this.mTableProperties = mTableProperties;
	}
	
	public void setTableProperty(HashMap<String, Object> mTableProperty) {
		if(this.mTableProperties == null ){
			this.mTableProperties = new ArrayList<HashMap<String, Object>>();
		}
		...
		this.mTableBandY.put(mTableProperty.get("tableId").toString(), Float.valueOf( mTableProperty.get("band_y").toString() ) );
	}
	...
	public void setColumnsArrayList(Object value ) {
		...
		this.mColumnAr = new ArrayList<String>();
		this.mOrderBy = new ArrayList<Boolean>();

		HashMap<String, String> _columnMap = null;
		
		for (int i = 0; i < retData.size(); i++) {
			_columnMap = (HashMap<String, String>) retData.get(i);
			_columns.add( _columnMap );
			this.mColumnAr.add(_columnMap.get("column"));
			this.mOrderBy.add( _columnMap.get("orderBy").toString().equals("true") );
		}
	...
	public void setFooterColumnsArray(Object value ) {
		JSONArray jsonArray = (JSONArray) value;
		List<Object> retData = Value.jsonArray2List( jsonArray );
		this.mColumnsAr = new ArrayList<String>();
		
		String _columnName = "";
		
		for (int i = 0; i < retData.size(); i++) {
			_columnName = (String) retData.get(i);
			this.mColumnsAr.add(_columnName);
		}
	}
